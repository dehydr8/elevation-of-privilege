...
it('retrieves player info for a game', async () => {
  ...
  response = await request(app.callback()).get(`/players/${response.body.game}`);
...
it('retrieve the model for a game', async () => {
  ...
  // retrieve the model
  response = await request(app.callback()).get(`/model/${response.body.game}`);
...
it('download the final model for a game', async () => {
  ...
  await server.db.set(`${gameName}:${gameID}`, state);
  await server.db.set(`${gameName}:${gameID}:metadata`, metadata);
  await server.db.set(`${gameName}:${gameID}:model`, model);

  // retrieve the model
  const response = await request(app.callback()).get(`/download/${gameID}`);
